...
> 为避免对不存在文档的依赖，本节改为“典型自研管道参考实现”的**示例性**对比，用于帮助理解取舍。

### 9.1 架构对比

| 特性 | LlamaIndex IngestionPipeline | 典型自研管道（参考实现） |
|------|------------------------------|--------------------------|
| **转换链** | ✅ 支持 | ✅ 可借鉴同类设计 |
| **缓存** | ✅ 自动（node+transform） | 常见：Redis/本地缓存（需自研策略） |
| **去重** | ✅ 自动（doc_id+hash） | 常见：doc_id + hash（需自研实现） |
| **增量更新** | ✅ 自动检测 | 常见：手动实现或基于元数据对账 |
| **向量存储** | ✅ 30+ 集成 | 常见：pgvector/专用向量库（按基建选型） |
| **并行处理** | ⚠️ 需要手动 | 可内置（取决于实现） |
| **自定义 Transform** | ✅ 简单 | ✅ 灵活（但需要工程约束） |

### 9.2 借鉴建议

```python
# 典型自研管道（参考实现示例：借鉴 LlamaIndex 的 pipeline 结构）
