...
class GitHubAPI:
    ...
    def get_file_urls(self, owner: str, repo: str, paths: List[str]) -> Dict[str, str]:
        """
        Get raw content URLs for multiple files.
        Useful for web_fetch to get source code.

        Returns dict mapping path to raw URL.
        """
        urls = {}
        for path in paths:
            urls[path] = f"https://raw.githubusercontent.com/{owner}/{repo}/main/{path}"
        return urls
    ...
    def search_code(
        ...
        Args:
            ...
            query: Search query (e.g., 'language:python filename:agent')
            limit: Max results

        Returns:
            List of matching code results with file paths
        """
        q = f"repo:{owner}/{repo} {query}"
        try:
            result = self._get(
                "/search/code", params={"q": q, "per_page": min(limit, 100)}
            )
    ...
    def get_github_blob_url(self, owner: str, repo: str, path: str,
                            start_line: int = None, end_line: int = None,
                            branch: str = "main") -> str:
        """
        Generate GitHub blob URL with optional line numbers.
        ...
        Returns:
            GitHub URL with optional line anchor

        Example:
            https://github.com/owner/repo/blob/main/path/to/file.py#L10-L20
        """
    ...
    def get_raw_url(self, owner: str, repo: str, path: str, branch: str = "main") -> str:
        """
...
def main():
    ...
    commands = {
        ...
        "search-code": lambda: api.search_code(owner, repo, sys.argv[4] if len(sys.argv) > 4 else ""),
    }
